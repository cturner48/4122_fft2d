...
{ 
  ...
  if(rank==0) {
	for(int round = 1; round < numtasks; round++){
	MPI_Request request;
	MPI_Isend(Htran + (round*w*numtasks), (2*height*w/nCPUs), MPI_COMPLEX, round, 0, MPI_COMM_WORLD, &request);	
	}
  } else {
	MPI_Status status;
	MPI_Recv(Htran, (2*height*w/nCPUs), MPI_COMPLEX, 0, 0, MPI_COMM_WORLD, &status);
  }
